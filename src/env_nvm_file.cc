...
Status NVMFile::Read(
  ...
  const uint64_t nbytes_remaining = GetFileSize() - offset;

  if (!nbytes_remaining) {              // End of file
    *result = Slice(scratch, 0);
    return Status::OK();
  } else if (nbytes_remaining < 0) {    // Bounds check
    return Status::IOError("Reading out of bounds");
  }
...
Status NVMFile::Allocate(uint64_t offset, uint64_t len) {
  ...
  if ((offset+len) > buf_len_) {
    uint64_t min_len = ((len / 4096) + 1) * 4096;
...
Status NVMFile::Append(const Slice& data) {
  ...
  uint64_t offset = GetFileSize() ? GetFileSize() -1 : 0;
...
Status NVMFile::Truncate(uint64_t size)
{
  ...
  char *huh = (char*)malloc(sizeof(*huh)*(fsize_+1));
  memcpy(huh, buf_, fsize_);
  huh[fsize_] = '\0';
  NVM_DEBUG("HERE IS WHAT I GOT(%s)\n", huh);
  free(huh);
